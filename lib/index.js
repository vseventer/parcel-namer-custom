...
var _default = new _plugin.Namer({
  async name({
    ...
    if (match) {
      ...
      const parsed = (0, _path.parse)(filePath); // Replace all macros except [hash], as this is internal Parcel macro we don't want to log.
      ...
      .replace(/\[name\]/gi, parsed.name).replace(/\[type\]/gi, bundle.type); // Log replacement.
